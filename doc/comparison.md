...
| Partial (in-progress) bins | The still-filling bin can be drawn area-true (`scale` mode) instead of misleadingly full-height or full-width ([`plot.data_until` / `setPartialBins`](api.md#partial-bins)) | Not a concept either platform's panel exposes |
